        ...
        >
          ...
          <ul v-if="!store.selectedPlayer" class="grid gap-2 m-0 p-3 list-none">
            <li v-for="player in store.nearbyPlayers" :key="player.id">
              ...
              >
                <span
                  class="grid flex-none place-items-center w-[39px] h-[39px] border-2 border-[rgba(255,255,255,0.35)] rounded-full text-white text-[11px] font-black"
                  :style="{ background: player.color }"
                  >{{ player.avatar }}</span
                >
                <span class="grid gap-[3px]"
                  ><strong class="text-sm">{{ player.name }}</strong
                  ><small class="text-[#b7cfb9] text-[10px]">Cấp {{ player.level }} · {{ player.title }}</small></span
                >
          ...
          <article v-else class="grid justify-items-center px-5 pt-4 pb-[22px] text-center">
            ...
            <span
              class="grid place-items-center w-[66px] h-[66px] my-2.5 border-2 border-[rgba(255,255,255,0.35)] rounded-full text-white text-lg font-black"
              :style="{ background: store.selectedPlayer.color }"
              >{{ store.selectedPlayer.avatar }}</span
            >
            <h3 class="m-0 text-xl">{{ store.selectedPlayer.name }}</h3><p class="mt-1 text-[#b7cfb9] text-xs">Cấp {{ store.selectedPlayer.level }} · {{ store.selectedPlayer.title }}</p>
            <div class="grid grid-cols-2 w-full gap-2 mt-[18px]">
              <span class="grid gap-1 py-[11px] px-1.5 rounded-[10px] bg-[rgba(7,28,18,0.38)] text-[#b7cfb9] text-[10px]"
                ><b class="text-[#f4f0df] text-[13px]">{{ store.selectedPlayer.caughtCount }}</b>Cá đã câu</span
              >
              <span class="grid gap-1 py-[11px] px-1.5 rounded-[10px] bg-[rgba(7,28,18,0.38)] text-[#b7cfb9] text-[10px]"
                ><b class="text-[#f4f0df] text-[13px]">{{ store.selectedPlayer.bestCatch }}</b>Kỷ lục</span
              >
            </div>
          </article>
        </section>
